...
export default function InvoicesPage() {
  ...
  const [items, setItems] = useState<InvoiceItem[]>([
    { id: 1, description: "Tooth Extraction", qty: 1, price: 50000, notes: "Deep root extraction" },
    { id: 2, description: "Teeth Whitening", qty: 1, price: 150000, notes: "Complete session" },
  ]);
  ...
  const handlePatientChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const patient = patients.find((entry) => entry.id === e.target.value);
  ...
  const saveInvoice = async () => {
    if (!invoice.patientId || !invoice.patientName || items.length === 0) return;
  ...
  const exportCSV = () => {
    ...
    let csv = "Invoice Number,Patient Name,Patient ID,Phone,Date,Items (Qty),Total (MMK)\n";
    todaysInvoices.forEach((inv) => {
      const itemDesc = inv.items.map((item) => `${item.description} (x${item.qty})`).join("; ");
      csv += `"${inv.invoiceNumber}","${inv.patientName}","${inv.patientId || ""}","${inv.patientPhone || ""}","${inv.date}","${itemDesc}",${inv.total}\n`;
    });
  ...
  const selectedPatient = patients.find((patient) => patient.id === invoice.patientId);

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-accent-charcoal dark:text-white">Invoices & Billing</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage billing, print cooler invoices, and export daily excel sheets.</p>
        </div>

        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-2xl w-full md:w-max">
          <button onClick={() => setActiveTab("create")} className={`flex-1 md:flex-none px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeTab === "create" ? "bg-white dark:bg-gray-700 text-accent-charcoal dark:text-white shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"}`}>
            Create Invoice
          </button>
          <button onClick={() => setActiveTab("history")} className={`flex-1 md:flex-none px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeTab === "history" ? "bg-white dark:bg-gray-700 text-accent-charcoal dark:text-white shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"}`}>
            Invoice History
          </button>
        </div>
      </div>
      ...
      {activeTab === "history" && (
        <Card className="p-0 overflow-hidden print:hidden">
          <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-gray-50/50 dark:bg-gray-900/10">
            ...
            <Button size="sm" onClick={exportCSV} className="gap-2 bg-emerald-600 hover:bg-emerald-700 focus:ring-emerald-600/20 text-white border-0 w-full sm:w-auto">
                    ...
                    <td className="py-4 px-6 dark:text-gray-300">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {inv.patientPhone ? inv.patientPhone : "No phone"}
                          {inv.patientId ? ` • ID #${inv.patientId.slice(-4)}` : ""}
                        </p>
                      </div>
                    </td>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 lg:gap-8 items-start">
          <div className="xl:col-span-4 space-y-6 print:hidden">
            <Card className="!p-5 space-y-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-muted ml-1">Patient</label>
                <select
                  value={invoice.patientId}
                  onChange={handlePatientChange}
                  className="w-full bg-surface-light border-2 border-border rounded-2xl px-4 py-3 text-base text-foreground outline-none transition-all focus:border-accent-primary focus:ring-4 focus:ring-accent-primary/15"
                >
                  <option value="">Select patient...</option>
                  {patients.map((patient) => (
                    <option key={patient.id} value={patient.id}>
                      {`${patient.firstName} ${patient.lastName}`} {patient.phone ? `- ${patient.phone}` : ""} {`(ID #${patient.id.slice(-4)})`}
                    </option>
                  ))}
                </select>
                <p className="text-xs text-muted ml-1">People with the same name are separated by phone number and patient ID.</p>
              </div>

              <Input label="Patient Name" name="patientName" value={invoice.patientName} onChange={handleInvoiceChange} readOnly />
              <Input label="Patient Phone" name="patientPhone" value={invoice.patientPhone} onChange={handleInvoiceChange} readOnly />
          ...
          <div className="xl:col-span-8">
            <Card className="font-sans shadow-2xl print:shadow-none print:p-0 print:border-none print:w-[800px] print:max-w-none bg-white overflow-hidden p-0 relative min-h-[800px] flex flex-col">
              ...
              <div className="p-5 sm:p-7 lg:p-10 pt-8 sm:pt-10 lg:pt-14 flex-1 flex flex-col">
                <div className="flex flex-col sm:flex-row justify-between items-start gap-6 pb-8">
                  <div>
                    <img
                      src="/logo.png"
                      alt="One Dental Specialist Clinic"
                      className="h-auto w-40 sm:w-48 lg:w-56 object-contain mb-4"
                    />
                  ...
                  <div className="text-right">
                    <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-gray-100 tracking-tighter uppercase mb-2">INVOICE</h3>
                ...
                <div className="mt-6 mb-8 lg:mb-12 flex gap-12 bg-gray-50 rounded-2xl p-4 sm:p-6 border border-gray-100 print:bg-transparent print:border-none print:p-0">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {invoice.patientPhone || selectedPatient?.phone || "No phone"}
                      {invoice.patientId ? ` • ID #${invoice.patientId.slice(-4)}` : ""}
                    </p>
                  </div>
                </div>
                ...
                <div className="min-h-[250px] flex-1">
                  <div className="overflow-x-auto">
                  <table className="w-full min-w-[560px] text-left mb-10">
                    <thead>
                ...
                <div className="flex justify-end pt-8 mb-12">
                  <div className="w-full sm:w-1/2 min-w-0 sm:min-w-[300px]">
